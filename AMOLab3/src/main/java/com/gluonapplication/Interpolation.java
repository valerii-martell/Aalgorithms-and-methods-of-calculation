...
public class Interpolation {
    ...
    public void setY(String str) {
        ...
        if (str.equals("sin(x^2)*exp(-(x^2)/4)")) {
            for (int i = 0; i <= count; i++) {
                yNodes[i] = Math.sin(Math.pow(xNodes[i], 2)) * Math.exp(-xNodes[i] * xNodes[i] / 4.0);
            }
        }
    }
    ...
    public double interp(String str, double xx) {
        ...
        if (str.equals("Aitken")) {
            result = (this.interpLagranj(xx)+this.interpNewton(xx))/2;
        }
